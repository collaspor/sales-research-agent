# MVP 验收清单

## 当前阶段

项目当前状态为：**POC 已完成，MVP 收敛中**。

POC 已验证核心 Agent 链路、证据可信度门禁、网页/PDF 摄取、异常降级和双格式报告。MVP 还需要证明售前工程师可以稳定、重复地使用这条链路。

## 验收门槛

| 验收项 | 当前状态 | 说明 |
|---|---|---|
| 海尔智家真实案例 | 部分通过 | 4 个来源成功，2 个来源 HTTP 403；报告明确披露失败 |
| 比亚迪真实案例 | 通过 | 6 个来源、5 条批准事实、0 个失败来源 |
| Fact 绑定 Evidence | 通过 | 无证据内容不能进入批准事实 |
| 来源网页可回查 | 通过 | HTML / Markdown 展示标题、URL 和来源等级 |
| PDF 解析 | 通过 | MinerU API 已完成真实调用验证 |
| 失败降级 | 通过 | 单来源失败不会阻断其他来源 |
| 离线回归测试 | 通过 | `97 passed, 1 deselected` |
| 本地交互入口 | 未完成 | 当前以 CLI 为主 |
| 连续运行稳定性 | 未完成 | 需要至少 5 次重复运行记录 |
| 售前人工审阅 | 进行中 | 需要对两个案例的事实逐条审阅并记录结论 |

## MVP 完成定义

满足以下条件后，才将项目状态改为“本地可用 MVP”：

1. 两个公开案例均可在 30 分钟预算内生成 HTML 和 Markdown；
2. 连续 5 次运行不出现流程级崩溃；
3. 每条批准 Fact 都能定位到 Evidence 和可点击来源 URL；
4. 403、超时、解析失败等失败均被记录，不被静默吞掉；
5. 售前工程师可以仅通过客户背景和研究目标启动调研；
6. 报告中的缺口、失败和来源等级能支持人工复核；
7. README、运行示例和验收记录与实际结果一致。

## 暂不纳入 MVP

- 多人账号和权限
- 云端部署
- MySQL 替换 SQLite
- 企业内部数据接入
- 自动生成完整售前方案或商务报价
- AIOps 和聊天功能
